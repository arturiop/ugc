...
const ChatPane = ({ onImageUploaded }: ChatPaneProps) => {
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        while (boundary !== -1) {
          ...
          if (eventName === "delta") {
            ...
            if (parsed.url) {
              ...
              setMessages((prev) =>
                prev.map((message) =>
                  message.id ===assistantMessageId ? { ...message, imageUrl: absoluteUrl } : message
                )
              );
            } else if (parsed.data) {
              ...
              setMessages((prev) =>
                prev.map((message) =>
                  message.id === assistantMessageId ? { ...message, imageUrl } : message
                )
              );
            }
      ...
      setMessages((prev) =>
        prev.map((item) => (item.id === assistantMessageId ? { ...item, content: message } : item))
      );
    } finally {
      setIsSending(false);
    }
