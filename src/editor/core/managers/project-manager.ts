...
import type {
	...
	TProjectSettings,
	TTimelineViewState,
} from "@/editor/types/project";
import type { ExportOptions, ExportResult, ExportState } from "@/editor/types/export";
import { storageService } from "@/editor/services/storage/service";
import { toast } from "sonner";
import { generateUUID } from "@/editor/utils/id";
import { UpdateProjectSettingsCommand } from "@/editor/lib/commands/project";
import { getProjectDurationFromScenes } from "@/editor/lib/scenes";
...
import { loadFonts } from "@/editor/lib/fonts/google-fonts";
...
export class ProjectManager {
	...
	async createNewProject({
		...
		const demoVideo = await createMockDemoVideoAsset();
		const newProject: TProject = createMockProject({
			id: projectId,
			name,
			demoVideo: demoVideo
				? {
						mediaId: demoVideo.id,
						duration: demoVideo.duration ?? 6,
						name: demoVideo.name,
				  }
				: undefined,
		});
		...
		if (demoVideo) {
			this.editor.media.setAssets({ assets: [demoVideo] });
		}
